...
class FalseFacer(Ideator):
    ...
    #TO DO:仮定Aについてのアイデアだけでなく、BとCについても出力する問題
    def _falseFaceFirstHalf(self)->Tuple[list,list]:
        ...
        #実際の挙動
        api_response=client.chat.completions.create(model=gpt_model,messages=message,max_tokens=4000,temperature=1.00).choices[0].message.content
        ...
        if len(idea_list)<10:
            ...
            raise Exception("InvalidDivideAtFalseFaceFHError")
    ...
    def _falseFaceSecondHalf(self,uans2_rev_assumption:str,count:int):
        ...
        if len(idea_list)<10:
            ...
            raise Exception("InvalidDivideAtFalseFaceLHError")
...
class IdeaBox(Ideator):
    ...
    def _enumParaAndAttForProblem(self)->Tuple[str,List[str]]:
        ...
        #処理部分
        att_response=client.chat.completions.create(model=gpt_model,messages=message,max_tokens=4000,temperature=1.00).choices[0].message.content
    ...
    def _generateIdeaUsingIdeaBox(self,parameters:str,selected_attribute:str,num:int):
        ...
        idea_response=client.chat.completions.create(model=gpt_model,messages=message,max_tokens=4000,temperature=1.00).choices[0].message.content
